...
#Modified Sequential Monte Carl ABC (ABC-SMC) with adaptive sequential importance sampling (also dubbed weighted-iterative ABC algorithm)
Weighted_iterative_ABC<- function(N_particles=1000,dimS=dim_summaries,
                                numCores=numCores,ABC_time_steps=10,seed_num=1,copula_type=copula_type){

  # N= total number of samples
  n_cores <- numCores   #Run on  n cores
  n<- N_particles/n_cores
  ...
  S_i <- NULL #saving saving stats for each simulation realisaton  
  ...
  x <- seq(from = -2, to = 15, length.out = 256)#range of prior distribution
  ...
  for (t in 1:ABC_time_steps) {
    ...
    if (t == 1) {
      pftn <- prior
      ABC_out <- mclapply(1:n_cores, ABC, pftn=pftn, n=n, mc.cores=n_cores)
     ...
     ##Sampling from the Multivariate Normal Perturbation kernel
      ...
      theta[[t]]<- as.matrix(do.call("rbind",as.list(theta_i)))# N_particles by dimS=11 matrix
    ...
    d[[t]]<-  do.call("c",as.list(d_i)) #length of N_particles
    
    small_draws<- epsilon[t]*N_particles #number of draw for posterior samples
    ...
    #print(paste("theta_main:"))
    #print(theta_main[[t]])
  
    
    eps[[t]]<- order(theta_dist[,(number_of_parameters+1)])[1:small_draws]#smallest distance index
    
    # choose posterior samples
    tha_post<-theta_dist[eps[[t]],][,-(number_of_parameters+1)]
    ...
    S<-na.omit(do.call("rbind",S_i))#combining the summary stats [N by 9 matrix]
    
    #current summary weights at time=t
    ## Summary weights based on inverse of variance
    #w1inv <- apply(S[d[[t]]<=eps_dist_max , ], 2, var, na.rm = TRUE)#inverse weight
    #############Summary weights based on inverse of mean absolute deviation
     #w1inv <- apply(S[d[[t]]<=eps_dist_max , ], 2, madstat, na.rm = TRUE)#inverse weight
    ...
    if (t == 1) {
      for (k in 1:number_of_parameters) { 
        fx[1,k,] <- density(theta_main[[1]][ ,k], from=-2, to=15, n=256)$y
        #saving the densities for each iteration
        write.csv(fx[1, ,], file = paste0("density_post_", 1,"_",N_particles, ".csv"))
      }
    }
    for (k in 1:number_of_parameters) {
      fx[t+1,k,] <- density(tha_post[, k], from=-2, to=15, n=256)$y
      
      #saving the densities for each iteration
      write.csv(fx[t+1, ,], file = paste0("density_post_", t+1,"_",N_particles,".csv"))
      
    }
    #saving importance weights 
    write.csv(import_weights[[t]], file = paste0("importance_weights_",t,"_",N_particles,".csv")) 
    
    #accepted particles at each iteration
    write.csv(tha_post, file = paste0("theta_post_", t,"_",N_particles, ".csv"))
    
    #saving weighted distance
    write.csv(d[[t]],file = paste0("weighted_distance_", t, "_",N_particles,".csv"))
  }
  
  timef<- proc.time()-time0
  CPUtime<-sum(as.vector(timef)[-3])
  write.csv(CPUtime,file=paste0("CPUtime_", N_particles, ".csv"))
